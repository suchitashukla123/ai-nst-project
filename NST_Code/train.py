...
def main():
    ...
    save_dir = PROJECT_DIR / 'experiment' / args.experiment
    ...
    content_dataset = ImageFolderDataset(
        args.content_dir, content_transform
    )

    style_dataset = ImageFolderDataset(
        args.style_dir, style_transform
    )
    ...
    if num_batches == 0:
        raise RuntimeError(
            'Dataset me batch_size ke liye enough images nahi hain.'
        )
